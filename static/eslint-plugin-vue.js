...
module.exports = {
  meta: {
    ...
    docs: {
      ...
      url:
        'https://github.com/vuejs/eslint-plugin-vue/blob/v5.0.0/docs/rules/component-name-in-template-casing.md'
    },
    fixable: 'code',
    schema: [
      {
  ...
  create(context) {
    ...
    const globalRegisteredComponentPatterns = (
      options.globalRegisteredComponentPatterns || []
    ).map(s => new RegExp(s))
    const tokens =
    ...
    function isVerifyTarget(node) {
      ...
      if (
        globalRegisteredComponentPatterns.some(
          re =>
            node.rawName.match(re) || casing.pascalCase(node.rawName).match(re)
        )
      ) {
        return true
      }
